...
Move AlphaBeta::get_move(State* state, int depth){
    ...
    for (long unsigned int i = 1; i <= action.size(); i++) {
        ...
        val = AlpBet(temp, depth-1, maximum, minimum, false);
        if (val > second_max) {
			int temp_val = i - 1;
            second_max = val;
          	best_move = temp_val;
...
int AlphaBeta::AlpBet(State* state, int depth, int alpha, int beta, bool maxPlayer) {
    ...
    if (maxPlayer) {
        ...
        for (long unsigned int i = 1; i <= action.size(); i++) {
            State* temp = state->next_state(action[i-1]);
            val = AlpBet(temp, depth-1, alpha, beta, false);
        ...
        for (long unsigned int i = 1; i <= action.size(); i++) {
            State* temp = state->next_state(action[i-1]);
            val = AlpBet(temp, depth-1, alpha, beta, true);
